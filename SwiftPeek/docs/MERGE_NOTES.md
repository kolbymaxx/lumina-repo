...
| #60 | `claude/ios-jailbreak-tweaks-cyei2i` | **0.4.0** | Icon inventory + M3 pixel signatures, hook-free hosting-view scan, **opt-in SpringBoard target**. Host analyser (`icons`, `tint-plan`) feeding Glyph. |
...
Both positions are defensible and they are not reconcilable by merge order —
someone has to decide. **Neither is more "recent" than the other; they were
written in parallel.**
